        ...
        klevu.each(klevu.search.extraSearchBox, function (key, box) {
            ...
            box.getScope().chains.events.focus.add({
                ...
                fire: function (data, scope) {
                    ...
                    klevu.each(klevu.search.extraSearchBox, function (key, box) {
                        var prevTarget = klevu.getSetting(box.getScope().settings, "settings.search.searchBoxTarget");
                        if (prevTarget) {
                            if (prevTarget.style.display == 'block'){
                                prevTarget.style = "display: none !important;";
                            }
                        }
                    });
            ...
            box.getScope().chains.template.render.add({
                ...
                fire: function (data, scope) {
                    var target = klevu.getSetting(scope.kScope.settings, "settings.search.searchBoxTarget");
                    var mySearchBoxId = scope.kScope.element.id;
                    var positions = scope.kScope.element.getBoundingClientRect();

                    if(mySearchBoxId == 'search1'){
                        klevu.dom.find(".klevuWrap", target)[0].style = "top:" + positions.top + "px; left: "+(positions.left - 450)+"px;right: auto;";
                    }else{
